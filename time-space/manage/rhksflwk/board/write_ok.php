...
if($MODE == "edit"){
	...
	if($i>0){
		...
		echo "<meta http-equiv='Refresh' content='0;url=view.php?now_page=$now_page&GUBN=$GUBN&SEARCH=$SEARCH&BOARD_ID=$BOARD_ID&SEQ=$BOARD_SEQ'>";
	}
	else
	{
...
if($MODE == "delete"){
	...
	if($i>0){
		...
		echo "<meta http-equiv='Refresh' content='0;url=list.php?BOARD_ID=$BOARD_ID'>";
	}
	else 
	{
...
if($_REQUEST['COMMENT_MODE'] == "write"){
	...
	if($i>0 || $I2>0){
		echo "<script type='text/javascript'>alert('SAVE OK.');</script>";
		echo "<meta http-equiv='Refresh' content='0;url=view.php?SEQ=$BOARD_SEQ&now_page=$now_page&GUBN=$GUBN&SEARCH=$SEARCH&BOARD_ID=$BOARD_ID&MODE=view'>";
	}
	else
	{
		echo "<script type='text/javascript'>alert('SAVE FAIL.');</script>";
...
if($_REQUEST['COMMENT_MODE'] == "delete"){
	...
	$proc_delete = "DELETE FROM T_BOARD_COMMENT WHERE SEQ = '$SEQ' AND BOARD_SEQ = '$BOARD_SEQ' AND BOARD_ID = '$BOARD_ID' AND USER_ID LIKE '%".$_SESSION['valid_user']."%'";
	$i = 0;
	$result = $GPLdb5->GPLexcute_query($proc_delete); //결과값 리턴
	$i = $GPLdb5->GPLmysql_affected_rows();
	if($i>0){
